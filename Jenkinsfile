pipeline { 
    agent any
    environment {
        DOCKER_HUB_REPO = "kamalsinghpadiyar/studybuddy"
    ...
    stages {
        stage('Checkout Github') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'github-token', url: 'https://github.com/KamalSPadiyar/STUDY_BUDDY_AI.git']])
            }
        }        
            ...
            steps {
                script {
                    kubeconfig(credentialsId: 'config', serverUrl: 'https://192.168.49.2:8443') {
                        sh '''
                        argocd login 35.238.134.199:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
